...
function Dashboard() {
  ...
  const handleExcelUpload = async (parsedData) => {
    try {
      await campusAPI.uploadArchitecture(parsedData);
      // Auto populate config panel values
      const totalBuildings = Object.keys(parsedData.buildings).length;
      const totalRooms = Object.keys(parsedData.rooms).length;

      setNumBuildings(totalBuildings);
      setNumRooms(Math.ceil(totalRooms / totalBuildings) || 0);

      alert("Architecture successfully uploaded and configured!");
    } catch (err) {
      console.error("Error uploading architecture", err);
      alert("Failed to upload architecture to the backend.");
    }
  };
      ...
      <div className="dashboard">
        ...
        <div className="pre-analysis-container">
          <div className="control-panel-main">
            <ExcelUpload onConfirm={handleExcelUpload} />
