...
export const campusAPI = {
  ...
  uploadArchitecture: (data) => api.post(`/campus/upload-architecture`, data),
};
...
// Person detection from uploaded image (YOLOv8)
export const detectionAPI = {
  /**
   * Upload an image and get a person count via YOLOv8.
   * @param {File} imageFile - JPEG/PNG image file
   * @returns {{ person_count, output_frame (base64), detection_details }}
   */
  detectFromImage: async (imageFile) => {
    const formData = new FormData();
    formData.append('image', imageFile);
    return api.post('/detection/detect-from-image', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 120000, // 120 s — model may need to download on first run
    });
  },
};
