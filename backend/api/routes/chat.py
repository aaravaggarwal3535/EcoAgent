...
class ChatRequest(BaseModel):
    ...
    session_token: Optional[str] = None  # Added for mobile torch control

from langchain_core.tools import tool
import asyncio

@tool
async def control_torch(session_token: str, action: str) -> str:
    """Controls the mobile phone torch/flashlight.
    Provide the session_token and action ("on" or "off").
    Only call this tool if a session token is provided.
    """
    from socket_server import sio
    
    print(f"[TOOL] control_torch called with session={session_token}, action={action}")
    
    # Emit async event to the room
    await sio.emit('torch_command', {'command': 'torch', 'action': action}, room=session_token)
    
    return f"Sent command to turn {action} the torch."
...
IMPORTANT: The data above contains all the information you need. Reference specific numbers when answering."""
        ...
        if response.tool_calls:
            for tool_call in response.tool_calls:
                if tool_call["name"] == "control_torch":
                    action_arg = tool_call["args"].get("action")
                    # Make sure we use the session token from the request
                    session_to_use = request.session_token
                    
                    if not session_to_use:
                        assistant_message = "I cannot turn on the torch because no mobile device is connected. Please click 'Connect Mobile' first."
                        break
                        
                    # Execute tool asynchronously
                    tool_result = await control_torch.ainvoke({"session_token": session_to_use, "action": action_arg})
                    
                    # You could optionally append the tool result and call LLM again to get a final conversational response:
                    messages.append(response)
                    messages.append({"role": "tool", "name": "control_torch", "content": tool_result, "tool_call_id": tool_call["id"]})
